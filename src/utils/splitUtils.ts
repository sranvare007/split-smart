...
export const goToPreviousDay = (split: Split): Split => {
  const prevIndex = split.currentDayIndex === 0 ? split.days.length - 1 : split.currentDayIndex - 1
  return {
    ...split,
 ...
 * Create a new split
 */
export const createSplit = (name: string, days: Array<{ dayNumber: number; muscleGroups: string }>, weeklyHolidays: number[]): Split => {
  return {
...
export const updateSplit = (splits: Split[], updatedSplit: Split): Split[] => {
  return splits.map((split) => (split.id === updatedSplit.id ? updatedSplit : split))
}
